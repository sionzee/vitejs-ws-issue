...
exports.vuePlugin = ({ root, app, resolver, watcher, config }) => {
    ...
    app.use(async (ctx, next) => {
        ...
        if (!query.type) {
            ...
            ctx.type = 'js';
            const { code, map } = await compileSFCMain(descriptor, filePath, publicPath);
            ctx.body = code;
            ctx.map = map;
        ...
        if (query.type === 'template') {
            ...
            ctx.type = 'js';
            const { code, map } = compileSFCTemplate(root, templateBlock, filePath, publicPath, descriptor.styles.some((s) => s.scoped), config.vueCompilerOptions);
            ctx.body = code;
            ctx.map = map;
            return etagCacheCheck(ctx);
        }
...
async function compileSFCMain(descriptor, filePath, publicPath) {
    ...
    if (descriptor.script) {
        ...
        if (descriptor.script.lang === 'ts') {
            ...
            content = code;
            descriptor.script.map = serverPluginSourceMap_1.mergeSourceMap(descriptor.script.map, JSON.parse(map));
        }
